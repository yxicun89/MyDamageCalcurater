...
func NewHandler(cfg Config) (http.Handler, error) {
	...
	g := &gateway{cfg: cfg}
	g.calcProxy = newReverseProxy(cfg.CalcURL, cfg.UpstreamTimeout, cfg.transport)
	if cfg.PokedexURL != nil {
		g.pokedexProxy = newReverseProxy(cfg.PokedexURL, cfg.UpstreamTimeout, cfg.transport)
	}
	if cfg.AssetsURL != nil {
		g.assetsProxy = newReverseProxy(cfg.AssetsURL, cfg.UpstreamTimeout, cfg.transport)
	}
...
func (g *gateway) serve(c *echo.Context) error {
	...
	// 許可オリジンの単純リクエストは、この先どの応答(上流の応答・gateway 自身のエラー)にも付ける。
	if allowed {
		setCORSAllowed(c.Response().Header(), origin)
	}

	// 2. ドットセグメントは拒否(どの上流にも送らない)。
	path := r.URL.Path
	if hasDotSegment(path) {
		return newError(api.NotFound, "%s", msgNotFound)
	}

	// 3. ルーティング(未知のパス・許さないメソッドはヘッダが無くても 404)。
	...
	case routePokedex:
		if g.pokedexProxy == nil {
			return newError(api.UpstreamUnavailable, "%s", msgUpstreamUnavailable)
		}
