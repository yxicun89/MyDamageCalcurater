...
//
// main は os.Getenv と pokedexdb.Provision・teamdb.Up・teamdb.Version・teamdb.DownAll を渡す。
...
// 「Up/Version/DownAll に team の関数を渡しているか」を検査できない(pokedex の関数を
// 誤って渡してもシグネチャが同じでコンパイルが通ってしまう。critic 1回目の指摘)。
// 関数ポインタの同一性で、team 以外の関数(pokedex の Up 等)を渡していないことを固定する。
func TestProductionEnvWiresTeamFunctions(t *testing.T) {
	env := productionEnv()
	cases := []struct {
		name string
		got  uintptr
		...
		{"Up", reflect.ValueOf(env.Up).Pointer(), reflect.ValueOf(teamdb.Up).Pointer()},
		{"Version", reflect.ValueOf(env.Version).Pointer(), reflect.ValueOf(teamdb.Version).Pointer()},
		{"DownAll", reflect.ValueOf(env.DownAll).Pointer(), reflect.ValueOf(teamdb.DownAll).Pointer()},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			if tc.got != tc.want {
				t.Errorf("productionEnv().%s が team 以外の関数を指している(pokedex の関数の誤配線の疑い)", tc.name)
			}
		})
	}
}
