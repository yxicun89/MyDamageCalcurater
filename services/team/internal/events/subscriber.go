...
// 実際の JetStream の durable consumer に繋ぐ。team-svc の障害は自分の中に収め、NATS に
// 接続できない・consumer を作れなくてもプロセスの起動を妨げない(CLAUDE.md 絶対ルール5。
// record-svc の internal/events/subscriber.go と同じ流儀)。
package events
...
const (
	...
	// handleTimeout は1件のメッセージの Handle(DB アクセスを含む)に与える上限(record-svc の
	// critic 指摘 R-8 の踏襲。ackWait より短くして、Nak を返せないまま応答不能になるより先にこちらで
	// 打ち切る)。
...
// consumerConfig は team-svc の durable consumer の設定(ADR-0209 §4)。
...
// Subscriber は team-svc から NATS JetStream(CALC_EVENTS ストリーム)への購読を1つにまとめる。
...
func New(natsURL string, handler *Handler) *Subscriber {
	...
	if err != nil {
		slog.Warn("team-svc: NATS に接続できない。購読を無効化する", "error", err)
		return nil
	}
	js, err := jetstream.New(nc)
	if err != nil {
		slog.Warn("team-svc: JetStream を初期化できない。購読を無効化する", "error", err)
		nc.Close()
		return nil
	}
...
func (s *Subscriber) tryStartConsuming(ctx context.Context) bool {
	...
	if err != nil {
		slog.Warn("team-svc: durable consumer を用意できない。再試行する", "error", err)
		return false
	}

	consumeCtx, err := consumer.Consume(s.onMessage, jetstream.ConsumeErrHandler(func(_ jetstream.ConsumeContext, err error) {
		slog.Warn("team-svc: 購読でエラーが発生した", "error", err)
	}))
	if err != nil {
		slog.Warn("team-svc: 購読を開始できない。再試行する", "error", err)
		return false
	}
...
func (s *Subscriber) onMessage(msg jetstream.Msg) {
	...
	if err != nil {
		slog.Error("team-svc: メッセージのメタデータを読めない。再配送させる", "error", err)
		if nakErr := msg.Nak(); nakErr != nil {
			slog.Warn("team-svc: Nak に失敗", "error", nakErr)
		}
		return
	}

	// DB がハングしても1メッセージの処理が無期限に詰まらないよう、AckWait より短い上限を付ける
	// (この上限を超えたら Nak 相当のまま応答が無くなり、AckWait 経過後にサーバーが自動で再配送する)。
	...
	if ackErr != nil {
		slog.Warn("team-svc: メッセージへの応答に失敗", "eventId", id, "action", action.String(), "error", ackErr)
	}
}

// EventID は JetStream のストリームシーケンスからログ用の識別子を組み立てる(重複排除には使わない。
// ADR-0213 §5)。
