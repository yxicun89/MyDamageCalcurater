...
// GET /metrics の統合テスト(ADR-0406 §1〜3。P7-1)。judge-svc の実際のハンドラ(New)に対して、
...
func TestMetricsCountsAPIRequests(t *testing.T) {
	...
	assertCountedOnce(t, h, metricsRequest{method: http.MethodGet, target: "/api/judge/healthz"}, "/api/judge/healthz")
	// 上流未設定・ヘッダ無しの判定はエラー応答になる。エラー応答も最終ステータスで数える。
	assertCountedOnce(t, h, metricsRequest{
		method: http.MethodPost, target: outspeedPath,
...
func TestMetricsDoesNotChangeUnknownRoutes(t *testing.T) {
	t.Parallel()
	h := New(Dependencies{})
	for _, target := range []string{"/metricsx", "/metrics/extra", "/api/judge/metrics"} {
