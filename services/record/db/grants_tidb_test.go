...
// RECORD_TEST_DSN(root 相当。CREATE USER・GRANT OPTION を持つこと)で、テスト専用の名前の
...
const (
	testAppUser      = "record_t_app"
	testMigratorUser = "record_t_migrator"
)
...
// testDSN は RECORD_TEST_DSN を検証して返す。
func testDSN(t *testing.T) (dsn string, cfg *mysql.Config) {
	...
	if err != nil {
		t.Fatalf("RECORD_TEST_DSN を解釈できない: %v", err)
	}
