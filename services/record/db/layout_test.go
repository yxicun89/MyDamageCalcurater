...
// requiredTables は ADR-0211 §6(ADR-0209 §3 #5・#5b)のテーブル。業務テーブル(calc_events・
// frequent_opponents・favorites)は P5-3 で追加した(ADR-0209 §3 #1〜#3・§5.2)。
var requiredTables = []string{"devices", "purge_journal", "calc_events", "frequent_opponents", "favorites"}
...
func TestDownAllRequiresConfirmation(t *testing.T) {
	const dsn = "testuser:testpass@tcp(127.0.0.1:1)/record_test"
	...
	cases := []struct {
		...
		{name: "確認なし", confirm: ""},
		{name: "別の DB 名", confirm: "record"},
		{name: "大文字小文字違い", confirm: "RECORD_TEST"},
		{name: "前方一致", confirm: "record_tes"},
