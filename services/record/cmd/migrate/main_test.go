...
// 実 DB に触らない(make test で走る)。実際の権限は services/record/db/grants_tidb_test.go。
//
// run のシグネチャ(実装への契約):
//
//	type cliEnv struct {
...
const (
	fakeProvisionDSN = "root:provPW0000000000000000@tcp(127.0.0.1:1)/record_fake"
	fakeAppDSN       = "record_app:appPW00000000000000000@tcp(127.0.0.1:1)/record_fake"
	fakeMigratorDSN  = "record_migrator:migrPW0000000000000000@tcp(127.0.0.1:1)/record_fake"
)
...
func fullProvisionEnv() map[string]string {
	return map[string]string{
		"RECORD_PROVISION_DSN": fakeProvisionDSN,
		"RECORD_APP_DSN":       fakeAppDSN,
		"RECORD_DATABASE_DSN":  fakeMigratorDSN,
	}
}

// RECORD_PROVISION_DSN が無ければプロビジョニングせず、RECORD_DATABASE_DSN で直接 Up する(後方互換)。
func TestUpWithoutProvisionDSNSkipsProvisioning(t *testing.T) {
	h := newHarness(map[string]string{"RECORD_DATABASE_DSN": fakeProvisionDSN})
	...
	if h.upDSN != fakeProvisionDSN {
		t.Error("Up に RECORD_DATABASE_DSN を渡していない")
	}
	h.assertNoSecrets(t)
}

// RECORD_PROVISION_DSN があれば、root で2ロール(app・migrator)をプロビジョニングしてから migrator で Up する。
func TestUpWithProvisionDSNProvisionsThenMigrates(t *testing.T) {
	...
	if h.upDSN != fakeMigratorDSN {
		t.Error("Up は migrator(RECORD_DATABASE_DSN)で行うこと。root(RECORD_PROVISION_DSN)で migrate しない")
	}
	h.assertNoSecrets(t)
}

// RECORD_PROVISION_DSN があるのに RECORD_APP_DSN が欠けていれば、何もせず終了コード 2(設定の誤り)。
func TestUpWithProvisionDSNRequiresAllRoleDSNs(t *testing.T) {
	...
	if !strings.Contains(h.stderr.String(), "RECORD_APP_DSN") {
		t.Errorf("stderr に欠けている環境変数名 RECORD_APP_DSN が無い: %q", h.stderr.String())
	}
	h.assertNoSecrets(t)
}

// RECORD_DATABASE_DSN が無ければ(PROVISION の有無によらず)何もせず失敗する(既存の挙動を保つ)。
func TestUpRequiresDatabaseDSN(t *testing.T) {
	env := fullProvisionEnv()
	delete(env, "RECORD_DATABASE_DSN")
	h := newHarness(env)
	if code := run([]string{"up"}, h.cliEnv()); code == 0 {
		t.Error("RECORD_DATABASE_DSN が無いのに成功した")
	}
	if len(h.calls) != 0 {
...
// プロビジョニングは up だけ。version は RECORD_PROVISION_DSN があってもプロビジョニングしない。
