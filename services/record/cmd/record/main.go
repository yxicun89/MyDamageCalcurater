// Command record は record-svc の唯一のバイナリ(ADR-0209・ADR-0211)。
//
//	record serve   # HTTP(/api/record/*・/healthz・/readyz)+ NATS JetStream の購読
//
// 設定は環境変数(config.go)。RECORD_APP_DSN は必須、RECORD_NATS_URL は任意
// (未設定なら購読を無効化するだけで起動は失敗しない。CLAUDE.md 絶対ルール5)。
// migration は別バイナリ(cmd/migrate。deploy/k8s の Job)が行う。この serve は DB へ
// sql.Open するだけで、起動時に migration を走らせない(pokedex-svc と同じ流儀)。
package main
...
import (
	...
	"example.com/pokecalc/services/record/internal/events"
	"example.com/pokecalc/services/record/internal/httpapi"
	"example.com/pokecalc/services/record/internal/store"
)

// HTTP タイムアウトと shutdown timeout(calc-svc・pokedex-svc と同じ考え方の値)。
const (
...
func run() int {
	...
	if err != nil {
		fmt.Fprintln(os.Stderr, "record serve:", err)
		return 1
	}

	db, err := openDB(cfg.DatabaseDSN)
	if err != nil {
		fmt.Fprintln(os.Stderr, "record serve: DB を開けない:", err)
		return 1
	}
	defer db.Close()

	st := store.New(db, cfg.DecayHalfLife, cfg.PurgeBatchLimit)
...
// shutdownTimeout 付きの Shutdown を呼ぶ(pokedex-svc の serve と同じ形)。
func serve(ctx context.Context, addr string, handler http.Handler) error {
	srv := newHTTPServer(addr, handler)

	errCh := make(chan error, 1)
	go func() {
		slog.Info("record-svc: 待ち受け開始", "addr", addr)
