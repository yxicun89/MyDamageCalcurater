...
// 生成型(api.*)と engine 型の変換、および Store を使った ID 解決(ADR-0016)。
//
...
// side・presets は engine の sentinel に一本化する(WASM 境界と同じ失敗にするため。ADR-0016 §4)。
...
// (engine 側が「持ち物なしの1通り」に既定するため。ADR-0016)。
...
// convertObservations は観測を検証して engine.Observation に写す(ADR-0016: キーの有無で数える)。
