...
// POST /api/calc/reverse の受け入れテスト(ADR-0200 AC-4、ADR-0010 §R)。
...
func TestCalcReverseErrors(t *testing.T) {
	...
	tests := []struct {
		...
		// HTTP ではキーの有無で数える: percent:0 は「指定したが範囲外」(ADR-0200)。
		...
		// side は WASM と同じく engine の sentinel に一本化する(列挙で先に弾くと invalid_enum になり食い違う。ADR-0200)。
