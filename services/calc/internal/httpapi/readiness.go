...
func deferredCalcHandler(current StoreFunc, publisher EventPublisher, pick func(*api.ServerInterfaceWrapper) echo.HandlerFunc) echo.HandlerFunc {
	return func(c *echo.Context) error {
		store := current()
		if store == nil {
			return errMasterUnavailable()
		}
		wrapper := &api.ServerInterfaceWrapper{Handler: NewServer(store, publisher)}
