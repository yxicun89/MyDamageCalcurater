...
|---|---:|---|---|
| `example.com/pokecalc/engine` | (replace `../../engine`) | 実数値・ランク補正の式(engine.RealStats / engine.EffectiveStat)。リポジトリ内モジュール | リポジトリ自身 |
...
repository自体のLICENSEはプロジェクト方針が決まるまで追加しない(balance と同じ)。
