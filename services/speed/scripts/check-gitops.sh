...
application_file=$speed_dir/deploy/argocd/application.yaml
overlay_file=$speed_dir/deploy/k8s/overlays/gitops/kustomization.yaml

fail() {
  echo "speed GitOps check failed: $1" >&2
  exit 1
}
...
# repoURL はアカウント名を含むので Git に書かない(ADR-0605 §2・ADR-0018)。ファイルは placeholder のままであることを必須にし、
# 適用時の値は ready モードでだけ SPEED_GITOPS_REPO_URL から受け取る(scripts/argocd-local-app.sh)。
...
if [ "$mode" = "ready" ]; then
  repo_url=${SPEED_GITOPS_REPO_URL:-$file_repo_url}
fi
...
[ "$source_path" = "services/speed/deploy/k8s/overlays/gitops" ] || fail "Application source path must use the GitOps overlay"
...
if grep -Eq '(^|[[:space:]])automated:' "$application_file"; then
  fail "automated sync must remain disabled for SP5"
fi

if [ "$mode" = "template" ]; then
  echo "speed GitOps template: valid"
  exit 0
fi
...
echo "speed GitOps configuration: ready"
