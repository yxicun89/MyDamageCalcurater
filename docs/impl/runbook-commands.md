...
| `read -rs PAT && kubectl -n argocd create secret generic repo-pokecalc --from-literal=type=git --from-literal=url="$(git remote get-url origin)" --from-literal=username=x-access-token --from-literal=password="$PAT" && kubectl -n argocd label secret repo-pokecalc argocd.argoproj.io/secret-type=repository` | balance §4、speed §6 | Git リポジトリの認証情報を Argo CD に登録(PAT は入力のみ・履歴に残さない) | host → k3d | **Secret 作成**(人間の作業。値は文書に書かない) |
