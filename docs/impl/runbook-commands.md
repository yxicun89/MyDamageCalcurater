...
| `make pokedex-registry-push` | data runbook「pokedex イメージを共有レジストリへ push する」(issue #237) | `docker build --target server` → `docker save` → balance-registry へ port-forward(5003)→ `crane push --insecure` → digest を出力 | host → docker → registry | image を push(kubectl context 検査あり。共有クラスタへは宛先を表示し `POKEDEX_REGISTRY_PUSH_CONFIRM=1` が無ければ中断。`POKEDEX_REGISTRY_HOST` で検証用に宛先切替可) |
