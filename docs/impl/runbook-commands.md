...
| `make deploy-latest` | verify §4 | `scripts/k3d-deploy-latest.sh`: context 検査 → mysql へ一時 port-forward し migrator の DSN で `make migrate-up` → pokedex-importer イメージを build・import(CronJob・import-k8s が使う)→ pokedex(server)を build・import・rollout restart → `api-k3d-deploy` → `web-k3d-deploy` → `judge-k3d-deploy` → read model があれば `balance-k3d-deploy-readmodel`・`speed-k3d-deploy-readmodel`(無ければ非ゼロで終了) | host → k3d(ns pokecalc) | 7 Deployment のイメージをいまのチェックアウトの内容に入れ替える |
