# ADR-0200: calc-svc の API 契約とマスタ境界
...
- `services/calc/internal/httpapi`: 生成物 `api.ServerInterface` を実装する。流れは wasmapi と同じ
  ...
  **pokedex の7操作(searchSpecies / getSpecies / searchMoves / getMove / getMovesByIds / searchItems / listNatures。
  getMove は2026-09-23・getMovesByIds は2026-09-24 追記。ADR-0105 §3)は生成ラッパ
