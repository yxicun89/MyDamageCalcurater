...
**既知の制限(critic指摘、2026-09-26)**: 一覧が `失敗` 状態のときに新規作成が成功すると、失敗の案内が消えて
「作成した1件だけの一覧」に見える(既存の構築が実は他にもあるかもしれないのに、それが出ていないことが伝わらない)。
実害は小さい(`list()` を呼び直すか画面を開き直せば正しい一覧に戻る)ため PR-A1 では対応を見送る。
対応するなら、`失敗` の案内を残したまま作成分を別枠で出す形が候補。
