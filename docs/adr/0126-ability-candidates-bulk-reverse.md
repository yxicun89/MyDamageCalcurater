...
## 追記(2026-09-25): 計算量の実測

独立レビュー(PR #402)で逆算の最悪ケース(持ち物 64・観測 16・特性 3)をネイティブで測ると、特性なし 8.9ms → 特性 3 つ 37.9ms(約 4.3 倍)だった。
「最大 3 倍」を上回るのは、特性ごとの全結果をためてから比較(reflect.DeepEqual)でまとめるため。ADR-0108 の目安(reverse 約 25ms)を超えるので、
WASM の最悪値の計測と、比較を特性ごとの逐次比較にして打ち切る最適化は後続で行う(結果は変わらない)。
