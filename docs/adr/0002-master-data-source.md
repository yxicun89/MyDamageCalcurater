...
| 4 | 食い違う技の裁定(calc のみの 11 技、Showdown のみの 1 技、タイプの食い違い 1 件。当初「2 件」としたのは誤り。追記 P2-1c) | **裁定済み(追記 P2-1c)**。方針回答: GameWith(ポケモンチャンピオンズ)・ポケモン徹底攻略などの**公式以外の攻略サイトも Web から調べてよい**。P2-1c で調査し、裁定を本 ADR に追記(規約・アクセス頻度に配慮) |
...
4. **食い違う技の裁定**(§3): calc のみの 11 技、Showdown のみの 1 技、タイプの食い違い 1 件(当初の「2 件」は誤り。**追記 P2-1c で裁定済み**)。ゲーム内または信頼できる出典で確認して補完ファイルに書く必要がある(個別名は差分スクリプトで生成)。
...
## 追記: P2-1c 技の使用可否の裁定(2026-09-21)

方針(ユーザー決定): 公式以外の攻略サイトも Web から調べてよい。使用不可と確認できた技は除外し、断定できない技は使用可として残す。
第三者データの一覧は本 ADR に載せない(件数・判定規則・出典のみ)。個別の技 ID は差分スクリプト(P2-2c)が `data/generated/` に出す。

### 調べ方

- calc 0.12.0 の Champions 世代と Showdown(`smogon/pokemon-showdown` commit f10d679、2026-09-20。champions mod)の技集合を突き合わせた。
  calc 525 件(番兵を除く)、Showdown で M-C に使える技(`isNonstandard` が null)515 件、共通 514 件。
- 共通 514 件で分類・威力の食い違いは 0 件(calc は変化技の `category` を省略するので、省略を Status として比べる)。
- 食い違った技を、公式以外の出典で確認した(robots.txt を守り、必要最小限の取得。取得した HTML は照合後に削除):
  Serebii(Champions の使用可能技一覧・技ごとのページ・Updated Attacks)、GameWith(ポケモンチャンピオンズの技一覧)、Bulbapedia。
  出典: https://www.serebii.net/pokemonchampions/moves.shtml 、 https://www.serebii.net/pokemonchampions/updatedattacks.shtml 、
  https://gamewith.jp/pokemon-champions/546417 、 https://bulbapedia.bulbagarden.net/ (確認日はすべて 2026-09-21)。
  Game8 は robots.txt で AI 系のクローラーを禁止しているため使っていない。

### 結論

| 区分 | 件数 | 裁定 | 根拠 |
|---|---|---|---|
| calc だけにある技 | 11 | **全件 使用不可**(除外) | calc 側は `type` / `category` の無い断片。Showdown では `isNonstandard: "Past"` で M-C に覚えるポケモンが 0。Serebii は「現在ゲームで使えない技」と明記、GameWith の技一覧にも無い |
| Showdown だけにある技 | 1 | **使用可**(マスタに残す) | Showdown で使用可、Serebii・GameWith の使用可能技一覧にもある。ただし M-C で覚えるポケモンは 0(選べるかは習得技で決まるので害は無い) |
| タイプの食い違い | 1 | **Showdown 側(Champions で変わったタイプ)が正しい** | Serebii の Updated Attacks・Bulbapedia・Showdown champions mod が一致。calc 0.12.0 と一部の攻略サイトは旧データのまま。**変化技なのでダメージには影響しない** |

- 当初「タイプの食い違い 2 件」としたのは誤り。本物は 1 件で、もう1件と数えたものは calc の断片か、§3 の「要約の読み方」の件だった。
- §3 の「ある攻略サイトの要約が、威力の変わる技 1 件のタイプを逆向きに読める」件は、calc・Showdown・Serebii・GameWith がすべて同じタイプで一致した。**要約の誤りと確定**(未確認事項を閉じる)。
- **ユーザーに裁定してもらう未確認の技は無い**。

### importer への規則(P2-2b / P2-2c)

1. 使える技 = calc と Showdown の両方にある技。calc の技のうち、`type` が無い断片、または Showdown champions で有効な `isNonstandard`(mod の上書きを優先、無ければ gen9)が null でないものは除外する。
2. Showdown にだけあり、`isNonstandard` が null の技は、Showdown の値で補って取り込む。
3. タイプは Showdown champions の上書きを優先する。calc と Showdown でタイプが食い違ったら差分として報告し、**変化技なら警告だけ、攻撃技なら取り込みを止めて人間の裁定を待つ**(ダメージに効くため)。
4. 分類の比較では calc の `category` 省略を Status として扱う。
5. 範囲外の気づき: 両方にあるが M-C で覚えるポケモンが 0 の技がいくつかある(使用可能技一覧にも無い)。「使用可能な技」を覚えるポケモンの有無で絞るかは P2-2 の設計で決める(既定案: マスタには置き、選べるかは習得技で決める)。
