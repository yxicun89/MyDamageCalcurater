...
- `GET /api/pokedex/natures` → 503 `upstream_unavailable`。**pokedex-svc を deploy/k8s に入れたら 200 の確認に変え**、
  ...
  **ADR-0206 で実施済み**: `GATEWAY_POKEDEX_URL`/`CALC_MASTER_URL` を base に設定。smoke は入手元(pokedex/example)を
  動的に判別する形になったため、`TestManifestGatewayLocalConfig` は「`GATEWAY_POKEDEX_URL` が base の既定 `http://pokedex`
  のまま(local で上書きしない)」の検査に移行した。
...
| AC-S4 | ~~local overlay は Component `api` を読む。calc のマスタと相性表は Component の直下のコピーから configMapGenerator で作った ConfigMap の読み取り専用マウントから来て…~~ **ADR-0206 で変更**: calc は base の `CALC_MASTER_URL=http://pokedex` で起動し(ConfigMap 経由のファイル方式は廃止)、Component `api` は gateway の patch(CORS・images)だけを持つ。gateway は base で起動でき、local では pokedex=`http://pokedex`(base のまま)・assets 未設定・CORS は `http://localhost:5173` だけ。イメージは `<repo>:local`。`deploy/k8s/overlays/local-api` は namespace `pokecalc`、resources は `../../base/calc`・`../../base/gateway` だけ、components は `../local/api` だけ(他レーンの resources を持たない) | `cmd/calc.TestManifestCalcBaseUsesPokedexMaster` / `TestManifestCalcLocalUsesPokedexMaster` / `TestManifestCalcLocalHasNoMasterCopy` / `TestManifestCalcLocalImage` / `cmd/gateway.TestManifestGatewayBaseConfig` / `TestManifestGatewayLocalConfig` / `TestManifestGatewayLocalImage` / `deploytest.TestLocalOverlayUsesAPIComponent` / `TestLocalAPIOverlayIsScopedToAPIServices` |
