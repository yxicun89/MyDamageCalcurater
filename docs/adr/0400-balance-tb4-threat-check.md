...
### 6. 細部(spec-writer が挙げた未決の確定。TB2・TB3 と揃える)
1. コアは技を `AnalyzeCoverage` と同じく検証する(1体あたり 4 件以下・同じ技の重複不可・分類は physical/special/status・攻撃技のタイプは 18 タイプ)。
   provider が不正な分類などを返したときは除外せずエラー(HTTP は 500 固定文言)。
2. 相性表が nil なら、攻撃技の有無によらず `ErrNilTypeChart`(ADR-0016 §6.3 と同じ)。
3. 特性の効果は、攻撃技の有無によらず両側とも全件を検証する(ADR-0017 §5・TB3 と同じ)。
4. request の `"abilityId": null` は省略と同じ(ADR-0017 §5.2)。
5. `moveIds` は必須(キーの欠落・`null` は 400)。空配列 `[]` は可(攻撃技なし)。ADR-0016 §6.1 と同じ。
6. `matchups` の各要素に入れるのは §2 の項目だけ(メンバーの abilityId・moveIds は入れない。request の順で対応が分かるため)。
7. 両側の全員のタイプ(1〜2 個・18 タイプ・重複なし)は、相手の攻撃技の有無によらず常に検証する(AnalyzeDefense と同じ。不正は HTTP 500)。
