...
- [ ] R-0 規約 v1 の策定と Codex レビュー(起草済み。Codex のレビュー結果を反映して確定し、AGENTS.md / CLAUDE.md から参照)
- [ ] R-1 監査: 規約違反の洗い出し(読み取り専用。ハードコード・可読性・公開可能性=秘密/個人情報/絶対パス/第三者データ・重複定義)。結果を一覧にして plan.md か docs/ に残す
- [ ] R-2 是正: 挙動を変えないリファクタリング(`make test` / `make test-golden` / `make test-wasm` / `make test-all-species` を全件維持)。1つの塊ごとに1コミット
