...
5. 候補の `pokemon` は、タイプの集合が候補と一致する(順不同)read model のポケモン全員を `pokemonId` の昇順で。`types` は read model の順のまま、
...
| Smoke | k3d(local overlay) | 既存の3つの example read model のマウントのまま、Ingress 経由で recommendations が 200(`"offenseHoles":[]`・`"types":["steel","fairy"]`・`"nameJa":"カソウメタル"`・`"abilityId":"ability-9002"` 等を含む) |

テストの ID・名前は架空(9001-000 / move-9001 / ability-9001 以降、`カソウ…`)を使う。実在ポケモン・技・特性の名前・ID を Git に置かない(ADR-0002)。
