...
## ユーザーの判断(2026-09-21 回答)
1. **LICENSE**(A-4): 法的に面倒なものは公開しない(個人で使うのは問題ない)。→ 現時点では LICENSE を置かない。公開するときに、法的に面倒なものを含まないと確認できた範囲で決める
2. **module path のアカウント名**(A-8): 公開しないようにプレースホルダに置き換える。→ R-2-8(`example.com/pokecalc/...`)
3. **Git 履歴の作者情報**(A-1): 履歴を書き換える。→ R-2-9(バックアップを取り、他の worktree・Codex の作業と調整してから実行)
4. **ADR-0002 の第三者データ抜粋**(A-6): 法的に面倒かを確認して判断してよい。→ 個別データ(技の威力変更表・持ち物名・特性名の一覧)を削除し、出典・方法・件数・結論だけにした
5. **タイプ相性表の正**(B-1): ハードコードは避け、変数(データ)にする。→ ADR-0012(タイプの一覧と相性表をマスタとして DB に登録し、engine には入力として渡す)。実装は P1-13
6. **engine の日本語ラベル**(B-5): 画面に表示されていればよく、持っていた方が扱いやすければ持ってよい。→ ADR-0009 §1-a
